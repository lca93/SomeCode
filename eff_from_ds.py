...
ws.factory('Gaussian::pGausDs(%s[1.8, 2.02],meanDs[1.97, 1.93, 1.99],pSigmaDs[0.012, 0.0001, 0.02])' %varName)
ws.factory('Gaussian::pGausDp(%s[1.8, 2.02],meanDp[1.87, 1.83, 1.89],pSigmaDp[0.012, 0.0001, 0.02])' %varName)
ws.factory('Gaussian::fGausDs(%s[1.8, 2.02],meanDs[1.97, 1.93, 1.99],fSigmaDs[0.012, 0.0001, 0.02])' %varName)
ws.factory('Gaussian::fGausDp(%s[1.8, 2.02],meanDp[1.87, 1.83, 1.89],fSigmaDp[0.012, 0.0001, 0.02])' %varName)

ws.factory('SUM::signalPass(pDs_yield[0, 1]*pGausDs,pGausDp)')
ws.factory('SUM::signalFail(fDs_yield[0, 1]*fGausDs,fGausDp)')

ws.factory('Chebychev::backgroundPass(%s, cPass[0,-1,1])' %varName)
ws.factory('Chebychev::backgroundFail(%s, cPass[0,-1,1])' %varName)
